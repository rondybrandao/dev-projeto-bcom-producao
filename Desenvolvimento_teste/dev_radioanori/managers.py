...
class CartManager(Manager):
    ...
    def add_cart_item(self, cart, ticket, quantity):
        cart_item = cart.cart_items.filter(cart=cart.pk, ticket=ticket).first()
        if cart_item:
            cart_item.quantity += quantity
            cart_item.unit_price = ticket.price
            cart_item.save()
            return cart_item
        return cart.cart_items.create(cart=cart.pk, ticket=ticket, quantity=quantity, unit_price=ticket.price)
